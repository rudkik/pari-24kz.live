...
<aside class="leagues p__leagues">
    <div class="hed-leag leagues__head">
        <button class="btn-self {{ request()->is('sport') ? '_js-active' : '' }}" type="button" aria-label="кнопка" data-r-btn="a1" onclick="window.location.href='/sport'">Линия</button>
        <button class="btn-self {{ request()->is('live') ? '_js-active' : '' }}" type="button" aria-label="кнопка" data-r-btn="a1" onclick="window.location.href='/live'">Live</button>
    </div>
				...
				@else
                  @foreach($arr_res_sport as $arr_res_spor)
                            <li class="list-leag__box-itm">
                              <a class="list-leag__itm" href="/bets/{{$arr_res_spor['id_sport']}}?category=line"> <span class="list-leag__ico">
                                <picture>
                        ...
                        @foreach($sport['events'] as $event)
                            <tr class="mch-tab__tr">
                                ...
                                @if(isset($event['b'][0]['oc_rate']))
                                    <td class="mch-tab__cast-td">
                                        <span id="sp{{ $event['b'][0]['oc_rate']  }}_127" onclick="selectKoef(this, '{{ $event['opp_1_name_ru'] }}','{{ $event['opp_2_name_ru'] }}', {{ $event['b'][0]['oc_rate']   }}, {{ $event['game_id'] }}, '{{ $event['b'][0]['oc_rate']   }}', 127)">

                                        {{ $event['b'][0]['oc_rate'] }}</span>
                                    </td>
                                @else
                                    <td class="mch-tab__cast-td"><span>-</span></td>
                                @endif
                                ...
                                @if(isset($event['m'][0]['oc_rate']))
                                    <td class="mch-tab__cast-td">
                                        <span id="sp{{ $event['m'][0]['oc_rate'] }}_128" onclick="selectKoef(this, '{{ $event['opp_1_name_ru'] }}','{{ $event['opp_2_name_ru'] }}', {{ $event['m'][0]['oc_rate']   }}, {{ $event['game_id'] }}, '{{ $event['m'][0]['oc_rate']   }}', 128)">
                                            {{ $event['m'][0]['oc_rate'] }}
                                        </span>
                                    </td>
                                @else
        ...
        @endif
            ...
            <style>
                @media (max-width: 480px) {
                    .coupon-as.active {
                        width: 100vw; /* Использование полной ширины экрана */
                        max-width: 100%; /* Ограничение ширины */
                        height: auto; /* Автоматическая высота */
                        position: fixed;
                        bottom: 0; /* Фиксировано внизу */
                        left: 0;
                    }
                }
                    ...
                    <div class="spoiler__box">
                        <form class="coupon__c-box c-cup" action="#!" method="POST">
                            ...
                            <div class="h1">
                                ...
                                <div class="info__sum" style="text-align: center; font-weight: bold">
