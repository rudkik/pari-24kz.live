...
<aside class="leagues p__leagues">
            ...
            <article class="spoiler" data-sel-target="">
                    ...
                    <div class="sport__c-box">
                        <div class="sport__top">
                            <section class="match _ibg sport__match">
                                <picture>
                                    ...
                                    <img class="_ibg-img" src="/img/grass.jpg" loading="lazy" width="100" height="100" alt="img"></picture>
                            ...
                            <section class="e-scroll">
                                <div class="e-scroll__btn-box">
                                    <div class="e-scroll__box-scrl">
                                        <button class="e-scroll__btn _js-active btn_cl_s" type="button" aria-label="кнопка"> Основное время
                                        </button>
                                        @foreach($gameone->sub_games as $key => $sub_games)
                                            <button class="e-scroll__btn btn_cl_s" type="button" aria-label="кнопка"> {{$sub_games->game_name}}</button>
                                        @endforeach
                                    </div>
                                    <script>
                                        $('.btn_cl_s').on('click', function() {
                                            $('.btn_cl_s').removeClass('_js-active');
                        ...
                        <div class="sport__box-2clm">
                                                ...
                                                @foreach($gameone->game_oc_list as $ls)
                                                    @if($ls->oc_group_name == '1x2')
                                                        <div class="line-1-sml">
                                                            ...
                                                            <div class="line-1-sml__r" id="sp{{$gameone->game_id}}_{{$i}}" onclick="selectKoef('{{$gameone->opp_1_name_ru}}','{{$gameone->opp_2_name_ru}}', {{$ls->oc_rate}}, {{$gameone->game_id}}, '{{$ls->oc_name}}', {{$i}})">
                                                                {{$ls->oc_rate}}
                                                            </div>
                                                        </div>
                                                            <?php $i++; ?>
                            ...
                            <div class="sport__colm">
                                <article class="sp-match spoiler _js-active sport__table" data-sel-target="">
                                    <div class="sp-match__head sp-match__head--sb">
                                        <h3 class="sp-match__ttl">Дополнительные исходы </h3>
                                        <button class="btn-v sp-match__v-btn spoiler__btn" type="button" aria-label="кнопка" data-btn-sel=""><img class="p-img-c" src="/img/ico/v2.svg" loading="lazy" width="100" height="100" alt="img"></button>
                                    </div>
                                    <div class="spoiler__hide">
                                        <ul class="list-sp-match">
                                            <li class="line-3-sml">
                                                @foreach($additional_outcomes as $outcome)
                                                    <div class="line-1-sml">
                                                        <div class="line-1-sml__l">
                                                            {{$outcome->name}}
                                                        </div>
                                                        <div class="line-1-sml__r" id="sp{{$gameone->game_id}}_{{$i}}" onclick="selectKoef('{{$gameone->opp_1_name_ru}}','{{$gameone->opp_2_name_ru}}', {{$outcome->odds}}, {{$gameone->game_id}}, '{{$outcome->name}}', {{$i}})">
                                                            {{$outcome->odds}}
                                                        </div>
                                                    </div>
                                                        <?php $i++; ?>
                    ...
                    <div class="spoiler__box">
                        <form class="coupon__c-box c-cup" action="#!" method="POST">
                            ...
                            <div class="c-cup__box-btn">
                                ...
                                <button class="btn c-cup__btn add_stavka_sport" type="button" aria-label="кнопка"> Сделать ставку
                                </button>
                            </div>
                        </form>
                    </div>
...
<script type="text/javascript">
    ...
    function sumK() {
        return arr_res_k.reduce((acc, k) => acc + k, 0).toFixed(2);
    }
