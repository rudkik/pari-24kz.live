        ...
        <aside class="leagues p__leagues">
            <div>
                <div class="hed-leag leagues__head">
                    <button class="btn-self" id="lineBtn" type="button" aria-label="кнопка" data-r-btn="a1" onclick="window.location.href='/sport'">Линия</button>
                    <button class="btn-self" id="liveBtn" type="button" aria-label="кнопка" data-r-btn="a1" onclick="window.location.href='/live'">Live</button>
                </div>
                ...
                <script>
                    ...
                    if (category === 'line') {
                        document.getElementById('lineBtn').classList.add('_js-active');
                    } else if (category === 'live') {
                        document.getElementById('liveBtn').classList.add('_js-active');
                    }
                </script>


                <label class="leagues__line search">
                    <input class="search__inp search_sports" type="text" name="search" placeholder="Поиск" autocomplete="off">
                    <span class="search__ico"><img class="p-img-c" src="/img/svg/search.svg" loading="lazy" width="100" height="100" alt="img"></span>
                </label>
                ...
                <article class="spoiler leagues__sect _js-active" data-sel-target="">
                    <div class="leagues__top" data-btn-sel="">
                        ...
                        <button class="btn-v spoiler__btn" type="button" aria-label="кнопка" data-go-btn="tl20">
                        ...
                        <ul class="leagues__list list-leag">
                            ...
                            @else
                                @foreach($data_leages as $data_leage)
                                    <li>
                                        <a class="list-leag__itm" href="/bets/{{$data_leage['sport_id']}}/{{$data_leage['id']}}">
                                            <span class="list-leag__ico">
                                                <picture>
                                                    <img class="p-img-c" src="/{{$data_leage['image']}}" loading="lazy" width="100" height="100" alt="иконка">
                                                </picture>
                                            </span>
                                            <span>{{$data_leage['name']}}</span>
                                        </a>
                                    </li>
                                @endforeach
                            @endif
            ...
            <section class="match-li">
                <div class="match-li__top">
                    <div class="match-li__ico">
                        <picture>
                            <img class="p-img-c" src="{{ '/' . ltrim($sport_img, '/') }}" loading="lazy" width="100" height="100" alt="img">
                        </picture>
                    </div>
                    <h2>{{ $title }}</h2>
                </div>
                        ...
                        <div class="wrap-tab-ml">
                            <table class="mch-tab match-li__tab">
                                ...
                                <tr class="mch-tab__tr">
                                    <th class="mch-tab__th">{{ $arr['name'] }}</th>
                                    <th>1</th>
                                    <th>X</th>
                                    <th>2</th>
                                    <th>1x</th>
                                ...
                                @foreach($arr['events'] as $event)
                                    ...
                                    <span>
                                        <a href="{">{{ $event['opp_2_name_ru'] }}</a>
                                    </span>
                                </span>
                                        </td>

                                        @foreach (['1x2', 'two', 'total', 'b', 'm'] as $type)
                                            @if(isset($event[$type][0]['oc_rate']))
                                                <td class="mch-tab__cast-td">
                                <span id="sp{{ $event['game_id'] }}_{{ $type }}"
                                      onclick="selectKoef('{{ $event['opp_1_name_ru'] }}','{{ $event['opp_2_name_ru'] }}', {{ $event[$type][0]['oc_rate'] }}, {{ $event['game_id'] }}, '{{ $event[$type][0]['oc_name'] }}', '{{ $type }}', this)">
                                    {{ $event[$type][0]['oc_rate'] }}
                                </span>
                                                </td>
                                            @else
                ...
                @if(isset($events_list))
                    <div class="wrap-tab-ml">
                        <table class="mch-tab match-li__tab">
                            ...
                            <tbody>
                            @foreach($events_list as $arr)
                                <tr class="mch-tab__tr">
                                    <td class="mch-tab__info" @if (Auth::check()) data-popup="#notificationPopup-match" @else data-popup="#pplogin"  @endif>
                                        <span>{{  $arr['game_start']  }}</span>
                                        <span class="mch-tab__i-name">
                                    <span class="mch-tab__ico">
                                        <picture>
                                            <img class="p-img-c" src="/img/ico/ico1.png" loading="lazy" width="100" height="100" alt="img">
                                    ...
                                    </td @if (!Auth::check()) data-popup="#pplogin"  @endif>
                                    @if(isset($arr['1x2'][0]['oc_rate']))
                                        <td class="mch-tab__cast-td">
                                    <span id="sp{{ $arr['game_id'] }}_12" onclick="selectKoef(this, '{{ $arr['opp_1_name_ru'] }}','{{ $arr['opp_2_name_ru'] }}', {{ $arr['1x2'][0]['oc_rate'] }}, {{ $arr['game_id'] }}, '{{ $arr['1x2'][0]['oc_name'] }}', 12)">
                                        {{ $arr['1x2'][0]['oc_rate'] }}
                                    </span>
                                        </td>
                                    @else
                                        <td class="mch-tab__cast-td"><span>-</span></td>
                                    @endif
                                    @if(isset($arr['1x2'][1]['oc_rate']))
                                        <td class="mch-tab__cast-td">
                                    <span id="sp{{ $arr['game_id'] }}_121" onclick="selectKoef(this, '{{ $arr['opp_1_name_ru'] }}','{{ $arr['opp_2_name_ru'] }}', {{ $arr['1x2'][1]['oc_rate'] }}, {{ $arr['game_id'] }}, '{{ $arr['1x2'][1]['oc_name'] }}', 121)">
                                        {{ $arr['1x2'][1]['oc_rate'] }}
                                    </span>
                                        </td>
                                    @else
                                    ...
                                    @if(isset($arr['1x2'][1]['oc_rate']))
                                        <td class="mch-tab__cast-td">
                                    <span id="sp{{ $arr['game_id'] }}_121" onclick="selectKoef(this, '{{ $arr['opp_1_name_ru'] }}','{{ $arr['opp_2_name_ru'] }}', {{ $arr['1x2'][1]['oc_rate'] }}, {{ $arr['game_id'] }}, '{{ $arr['1x2'][1]['oc_name'] }}', 122)">
                                        {{ $arr['1x2'][1]['oc_rate'] }}
                                    </span>
                                        </td>
                                    @else
                                        <td class="mch-tab__cast-td"><span>-</span></td>
                                    @endif
                                    @if(isset($arr['two'][0]['oc_rate']))
                                        <td class="mch-tab__cast-td">
                                    <span id="sp{{ $arr['game_id'] }}_123" onclick="selectKoef(this, '{{ $arr['opp_1_name_ru'] }}','{{ $arr['opp_2_name_ru'] }}', {{ $arr['two'][0]['oc_rate'] }}, {{ $arr['game_id'] }}, '{{ $arr['two'][0]['oc_name'] }}', 123)">
                                        {{ $arr['two'][0]['oc_rate'] }}
                                    </span>
                                        </td>
                                    @else
                                        <td class="mch-tab__cast-td"><span>-</span></td>
                                    @endif
                                    @if(isset($arr['two'][1]['oc_rate']))
                                        <td class="mch-tab__cast-td">
                                    <span id="sp{{ $arr['game_id'] }}_124" onclick="selectKoef(this, '{{ $arr['opp_1_name_ru'] }}','{{ $arr['opp_2_name_ru'] }}', {{ $arr['two'][1]['oc_rate'] }}, {{ $arr['game_id'] }}, '{{ $arr['two'][1]['oc_name'] }}', 124)">
                                        {{ $arr['two'][1]['oc_rate'] }}
                                    </span>
                                        </td>
                                    @else
                                        <td class="mch-tab__cast-td"><span>-</span></td>
                                    @endif
                                    @if(isset($arr['two'][2]['oc_rate']))
                                        <td class="mch-tab__cast-td">
                                    <span id="sp{{ $arr['game_id'] }}_125" onclick="selectKoef(this, '{{ $arr['opp_1_name_ru'] }}','{{ $arr['opp_2_name_ru'] }}', {{ $arr['two'][2]['oc_rate'] }}, {{ $arr['game_id'] }}, '{{ $arr['two'][2]['oc_name'] }}', 125)">
                                        {{ $arr['two'][2]['oc_rate'] }}
                                    </span>
                                        </td>
                                    @else
                                        <td class="mch-tab__cast-td"><span>-</span></td>
                                    @endif
                                    @if(isset($arr['total'][0]['oc_rate']))
                                        <td class="mch-tab__cast-td">
                                    <span class="mch-tab__grid">
                                    <span id="sp{{ $arr['game_id'] }}_126" onclick="selectKoef(this, '{{ $arr['opp_1_name_ru'] }}','{{ $arr['opp_2_name_ru'] }}', {{ $arr['total'][0]['oc_rate'] }}, {{ $arr['game_id'] }}, '{{ $arr['total'][0]['oc_rate'] }}', 126)">
                                            {{ $arr['total'][0]['oc_rate'] }}
                                        </span>
                                    ...
                                    @else
                                        <td class="mch-tab__cast-td"><span>-</span></td>
                                    @endif
                                    @if(isset($arr['b'][0]['oc_rate']))
                                        <td class="mch-tab__cast-td">
                                    <span id="sp{{ $arr['game_id'] }}_127" onclick="selectKoef(this, '{{ $arr['opp_1_name_ru'] }}','{{ $arr['opp_2_name_ru'] }}', {{ $arr['b'][0]['oc_rate'] }}, {{ $arr['game_id'] }}, '{{ $arr['b'][0]['oc_rate'] }}', 127)">
                                                {{ $arr['b'][0]['oc_rate'] }}
                                            </span>
                                        </td>
                                    @else
                                        <td class="mch-tab__cast-td"><span>-</span></td>
                                    @endif
                                    @if(isset($arr['m'][0]['oc_rate']))
                                        <td class="mch-tab__cast-td">
                                    <span id="sp{{ $arr['game_id'] }}_127" onclick="selectKoef(this, '{{ $arr['opp_1_name_ru'] }}','{{ $arr['opp_2_name_ru'] }}', {{ $arr['m'][0]['oc_rate'] }}, {{ $arr['game_id'] }}, '{{ $arr['m'][0]['oc_rate'] }}', 128)">
                                                {{ $arr['m'][0]['oc_rate'] }}
                                            </span>
                                        </td>
                                    @else
                    ...
                    @foreach($arry as $sport)
                        <div class="sport-section">
                            <div class="sport-header">
                                <img src="{{ asset($sport_img) }}" alt="{{ $title }}" class="sport-icon">
                                <h2>{{ $title }}</h2>
                            </div>
                                ...
                                <div class="event-card">
                                    ...
                                    <div class="event-odds">
                                        @if(isset($event['1x2'][0]['oc_rate']))
                                            <div class="odd" onclick="selectKoef(this, '{{ $event['opp_1_name_ru'] }}','{{ $event['opp_2_name_ru'] }}', {{ $event['1x2'][0]['oc_rate'] }}, {{ $event['game_id'] }}, '{{ $event['1x2'][0]['oc_name'] }}', 120)">
                                                {{ $event['1x2'][0]['oc_rate'] }}
                                            </div>
                                        @else
                                            <div class="odd">-</div>
                                        ...
                                        @if(isset($event['m'][0]['oc_rate']))
                                                <div class="odd" onclick="selectKoef(this, '{{ $event['opp_1_name_ru'] }}','{{ $event['opp_2_name_ru'] }}', {{ $event['m'][0]['oc_rate'] }}, {{ $event['game_id'] }}, '{{ $event['m'][0]['oc_rate'] }}', 122)">
                                                {{ $event['m'][0]['oc_rate'] }}
                                            </div>
                                        @else
                                            <div class="odd">-</div>
                    ...
                    <div class="sport-section">
                        ...
                        @foreach($events_list as $arr)
                            <div class="event-card">
                                ...
                                <div class="event-odds">
                                    @if(isset($arr['1x2'][0]['oc_rate']))
                                        <div class="odd" onclick="selectKoef(this, '{{ $arr['opp_1_name_ru'] }}','{{ $arr['opp_2_name_ru'] }}', {{ $arr['1x2'][0]['oc_rate'] }}, {{ $arr['game_id'] }}, '{{ $arr['1x2'][0]['oc_name'] }}', 12)">
                                            {{ $arr['1x2'][0]['oc_rate'] }}
                                        </div>
                                    @else
                                        <div class="odd">-</div>
                                    ...
                                    @if(isset($arr['1x2'][1]['oc_rate']))
                                        <div class="odd" onclick="selectKoef(this, '{{ $arr['opp_1_name_ru'] }}','{{ $arr['opp_2_name_ru'] }}', {{ $arr['1x2'][1]['oc_rate'] }}, {{ $arr['game_id'] }}, '{{ $arr['1x2'][1]['oc_name'] }}', 121)">
                                            {{ $arr['1x2'][1]['oc_rate'] }}
                                        </div>
                                    @else
                                        <div class="odd">-</div>
                                    ...
                                    @if(isset($arr['m'][0]['oc_rate']))
                                            <div class="odd" onclick="selectKoef(this, '{{ $arr['opp_1_name_ru'] }}','{{ $arr['opp_2_name_ru'] }}', {{ $arr['m'][0]['oc_rate'] }}, {{ $arr['game_id'] }}, '{{ $arr['m'][0]['oc_rate'] }}', 122)">
                                            {{ $arr['m'][0]['oc_rate'] }}
                                        </div>
                                    @else
                                        <div class="odd">-</div>
        ...
        <aside  class="coupon-as" >
            <article class="coupon coupon-as__cup spoiler _js-active" data-go-target="c1">
                <div class="coupon__top">
                    ...
                    <button class="btn-v spoiler__btn" type="button" aria-label="кнопка" data-go-btn="c1">
                        <img class="p-img-c" src="/img/ico/v2.svg" loading="lazy" width="100" height="100" alt="img">
                    </button>
                </div>
                    ...
                    <form class="coupon__c-box c-cup" action="#!" method="POST">
                        ...
                        <div class="h1">
                            ...
                            <div class="amounts-div">
                                <button class="preset" onclick="setPresetValue(100, event)">100</button>
                                <button class="preset" onclick="setPresetValue(200, event)">200</button>
                                <button class="preset" onclick="setPresetValue(500, event)">500</button>
                                <button class="preset" onclick="setPresetValue(700, event)">700</button>
                                <button class="preset" onclick="setPresetValue(1000, event)">1000</button>
                                <button class="preset" onclick="setPresetValue(2000, event)">2000</button>
                                <button class="preset" onclick="setPresetValue(5000, event)">5000</button>
                            </div>
                            <div class="info__sum" style="text-align: center; margin-top: 30px; font-weight: bold">
                        ...
                        <div class="c-cup__box-btn">
                            ...
                            <label class="lbl-acp coupon__acpt" for="check11">
                                <span>Всегда соглашаться с изменением коэффициента</span>
                            </label>
                            <button class="btn c-cup__btn" type="button" aria-label="кнопка" @if (!Auth::check()) data-popup="#pplogin"  @endif> Сделать ставку</button>
                        </div>
                    </form>
                </div>
            </article>
...
<script type="text/javascript">
    ...
    function closeClick(fc, k, game_id, fdc) {
        ...
        if (!hasBets) {
            $('.h1').hide();
            $('.c-cup__ttl.coupon__ttl').show(); // показать текст "Выберите событие", если нет ставок
        }
    ...
    $('.c-cup__btn').on('click', function(){
        ...
        $.ajax({
            ...
            method: 'post',
            dataType: 'json',
            data: {'_token':'{{csrf_token()}}', arr_res_k: arr_res_k, arr_res_game_id: arr_res_game_id, arr_team: arr_team, price: price },
